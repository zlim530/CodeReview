...
/* 
索引 Index
·全表扫描：对数据进行检索（select）效率最差的是全表扫描，即一条条的找
·如果没有目录，查汉语字典就要一页页的翻，而有了目录只要查询目录即可。
为了提高检索的速度，可以为经常进行检索的列添加索引，相当于创建目录。
·使用索引能提高查询效率，但是索引也是占用空间的，而且添加、更新、删除数据
的时候也需要同步更新索引，因此会降低Insert、Update、Detele的速度。只在
经常检索的字段上（where）创建索引。
...
1.索引的目的：提高查询效率
2.索引种类：
	2.1聚集索引（物理），一个表只能又一个聚集索引；
	2.2非聚集索引（逻辑），一个表中可以有多个非聚集索引
		非聚集索引的顺利可能与表实际在物理内存空间中的存储顺序不一致；
3.增加索引后，会增加额外的存储空间，同时会降低增加、修改、删除纪录的效率。
4.预读取、物理读取、逻辑读取：
	·SQL Sever存储数据都是以页为单位（每页大小8KB）
	·逻辑读取：从缓存中读取数据
	·物理读取：从磁盘中读取数据
	·预读取：一种性能优化机制，在执行查询时先预测执行“查询计划”所需的数据
	和索引也，然后在查询实际使用这些页之前将它们读入缓存区高速缓存。
*/
select c3,c4 from TestIndex
where c4 > 800 and c4 < 1000 order by c4 asc

--创建索引
create clustered index IXc4 on TestIndex(c4)
--删除索引
drop index TestIndex.IXc4